...
export class ServeurApplications {
	...
	demarrer(): void {
		let ceServeur = this;
		this.serveur = this.appli.listen(this.port, () => {
			console.log(
				'* ' + creerDateMaintenant().representationLog() + ' - Le serveur écoute le port ' + this.port + " de l'hôte (local ou heroku)."
			);
		});
	}
	/*
    ...
    */
	...
	definirParametrisationVuesDynamique(suffixe: string, rep: string, cles: string[]): void {
		this.appli.engine(
			suffixe,
			(chemin: string, remplacement: { [cle: string]: string }, continuation: (err: NodeJS.ErrnoException, rendu?: string) => string) => {
	...
	enregistrerReponseARequeteGET(chemin: string, calcul: (i: Interaction) => void): void {
		this.appli.get(chemin, (requete: express.Request, reponse: express.Response, suite: express.NextFunction) => {
